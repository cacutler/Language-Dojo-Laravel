...
class ProgressTrackingController extends Controller {
    ...
    public function index(Request $request) {
        ...
        return view('progress-tracking.index', ['progressTracking' => $progressTracking->paginate()]);
    }
    /**
     ...
     */
    public function destroy(Request $request, ProgressTracking $progressTracking) {
        $this->authorize('delete', $progressTracking);
        $progressTracking->forceDelete();
        if ($request->expectsJson()) {
            return response()->noContent();
        }
        return redirect()->route('web.progress-tracking.index')->with('status', 'Progress deleted.');
    }
}
