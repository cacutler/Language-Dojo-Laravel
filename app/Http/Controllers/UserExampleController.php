...
class UserExampleController extends Controller {
    ...
    public function index(Request $request) {
        ...
        return view('user-examples.index', ['userExamples' => $userExamples->paginate()]);
    }
    /**
     ...
     */
    public function destroy(Request $request, UserExample $userExample) {
        $this->authorize('delete', $userExample);
        $userExample->forceDelete();
        if ($request->expectsJson()) {
            return response()->noContent();
        }
        return redirect()->route('web.user-examples.index')->with('status', 'Example deleted.');
    }
}
